...
class CollectorDatabase:
    ...
    def _ensure_global_tables(self) -> None:
        self.connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS symbol_orderbook_status (
                ...
                current_price REAL NOT NULL, bid_price REAL NOT NULL, ask_price REAL NOT NULL,
                spread REAL NOT NULL, bid_volume REAL NOT NULL, ask_volume REAL NOT NULL,
                bid_total_volume REAL NOT NULL, ask_total_volume REAL NOT NULL,
                book_imbalance REAL NOT NULL, bid_imbalance REAL NOT NULL,
                ask_imbalance REAL NOT NULL, imbalance_ratio REAL NOT NULL,
                orderbook_pressure REAL NOT NULL, spread_pct REAL NOT NULL,
                vwap_5 REAL NOT NULL, liquidity_density REAL NOT NULL,
                PRIMARY KEY (exchange, symbol)
            );
            CREATE TABLE IF NOT EXISTS account_snapshots (
                exchange TEXT NOT NULL, timestamp TEXT NOT NULL,
                total_value REAL NOT NULL, balances_json TEXT NOT NULL,
                PRIMARY KEY (exchange, timestamp)
            );
            CREATE TABLE IF NOT EXISTS collector_events (
                id INTEGER PRIMARY KEY AUTOINCREMENT, collector TEXT NOT NULL,
    ...
    def ensure_market_tables(self, exchange: str, symbol: str) -> None:
        ...
        self.connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS {prefix}_candles (
                timestamp TEXT NOT NULL, candle_interval TEXT NOT NULL,
                open_price REAL NOT NULL, high_price REAL NOT NULL,
                low_price REAL NOT NULL, close_price REAL NOT NULL,
                volume REAL NOT NULL, trade_amount REAL NOT NULL DEFAULT 0,
                PRIMARY KEY (timestamp, candle_interval)
            );
            CREATE TABLE IF NOT EXISTS {prefix}_trades (
                id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT NOT NULL,
                price REAL NOT NULL, volume REAL NOT NULL, total_value REAL NOT NULL,
                is_buyer_maker INTEGER NOT NULL, vwap REAL NOT NULL,
                UNIQUE(timestamp, price, volume, total_value, is_buyer_maker)
            );
            CREATE TABLE IF NOT EXISTS {prefix}_orderbooks (
                timestamp TEXT PRIMARY KEY, bid_price REAL NOT NULL,
                bid_volume REAL NOT NULL, ask_price REAL NOT NULL,
                ask_volume REAL NOT NULL, bid_total_volume REAL NOT NULL,
                ask_total_volume REAL NOT NULL, spread REAL NOT NULL,
                book_imbalance REAL NOT NULL, bid_imbalance REAL NOT NULL,
                ask_imbalance REAL NOT NULL, imbalance_ratio REAL NOT NULL,
                orderbook_pressure REAL NOT NULL, spread_pct REAL NOT NULL,
                vwap_5 REAL NOT NULL, liquidity_density REAL NOT NULL
            ...
            CREATE TABLE IF NOT EXISTS {prefix}_orderbook_levels (
                level INTEGER PRIMARY KEY, side TEXT NOT NULL, price REAL NOT NULL,
                quantity REAL NOT NULL, total REAL NOT NULL,
                current_price REAL NOT NULL, timestamp TEXT NOT NULL
            );
            """
        )
        self.connection.commit()
    ...
    def ensure_indicator_table(self, exchange: str, symbol: str, interval: str) -> str:
        ...
        self.connection.execute(
            f"CREATE TABLE IF NOT EXISTS {table} (timestamp TEXT PRIMARY KEY, payload_json TEXT NOT NULL)"
        )
        self.connection.commit()
        return table
    ...
    def save_candles(self, exchange: str, symbol: str, interval: str, rows: list[dict]) -> int:
        ...
        for row in rows:
            self.connection.execute(
                ...
                """,
                (
                    row["timestamp"].isoformat(), interval, row["open"], row["high"],
                    row["low"], row["close"], row["volume"],
                    float(row.get("trade_amount", row["close"] * row["volume"])),
                ),
            )
        self.connection.commit()
        return len(rows)
    ...
    def get_candles(self, exchange: str, symbol: str, interval: str, limit: int = 200) -> list[dict]:
        ...
        rows = self.connection.execute(
            ...
            FROM {table} WHERE candle_interval = ? ORDER BY timestamp DESC LIMIT ?
            """,
            (interval, limit),
        ).fetchall()
    ...
    def save_trades(self, exchange: str, symbol: str, rows: list[dict]) -> int:
        ...
        for row in rows:
            cursor = self.connection.execute(
                ...
                """,
                (
                    row["timestamp"].isoformat(), round(float(row["price"]), 8),
                    round(float(row["volume"]), 8),
                    round(float(row.get("total_value", row["price"] * row["volume"])), 8),
                    int(row.get("is_buyer_maker", 0)), round(float(row.get("vwap", 0)), 8),
                ),
            )
    ...
    def save_orderbook(self, exchange: str, symbol: str, row: dict) -> None:
        ...
        self.connection.execute(
            ...
            (timestamp, bid_price, bid_volume, ask_price, ask_volume,
             bid_total_volume, ask_total_volume, spread, book_imbalance,
             bid_imbalance, ask_imbalance, imbalance_ratio, orderbook_pressure,
             spread_pct, vwap_5, liquidity_density)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                row["timestamp"].isoformat(), row["bid_price"], row["bid_volume"],
                row["ask_price"], row["ask_volume"], row["bid_total_volume"],
                row["ask_total_volume"], row["spread"], row["book_imbalance"],
                row["bid_imbalance"], row["ask_imbalance"], row.get("imbalance_ratio", 0),
                row.get("orderbook_pressure", 0), row.get("spread_pct", 0),
                row.get("vwap_5", 0), row.get("liquidity_density", 0),
            ),
        )
        self.connection.commit()
    ...
    def save_orderbook_depth(self, exchange: str, symbol: str, row: dict, levels: int = 30) -> None:
        ...
        values = [
            (-(levels - index), "bid", price, quantity, price * quantity, current_price, timestamp)
            for index, (price, quantity) in enumerate(bids)
        ]
        values.append((0, "mid", current_price, 0.0, 0.0, current_price, timestamp))
        values.extend(
            (index + 1, "ask", price, quantity, price * quantity, current_price, timestamp)
    ...
    def save_orderbook_status(self, exchange: str, symbol: str, row: dict) -> None:
        ...
        self.connection.execute(
            ...
            (exchange, symbol, timestamp, current_price, bid_price, ask_price,
             spread, bid_volume, ask_volume, bid_total_volume, ask_total_volume,
             book_imbalance, bid_imbalance, ask_imbalance, imbalance_ratio,
             orderbook_pressure, spread_pct, vwap_5, liquidity_density)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                exchange, symbol, row["timestamp"].isoformat(), current_price,
                row["bid_price"], row["ask_price"], row["spread"], row["bid_volume"],
                row["ask_volume"], row["bid_total_volume"], row["ask_total_volume"],
                row["book_imbalance"], row["bid_imbalance"], row["ask_imbalance"],
                row.get("imbalance_ratio", 0), row.get("orderbook_pressure", 0),
                row.get("spread_pct", 0), row.get("vwap_5", 0),
                row.get("liquidity_density", 0),
            ),
        )
        self.connection.commit()
    ...
    def save_account_snapshot(self, exchange: str, total_value: float, balances: list[dict]) -> None:
        self.connection.execute(
            ...
            (
                exchange, datetime.utcnow().replace(microsecond=0).isoformat(), total_value,
    ...
    def log_event(self, collector: str, event_type: str, status: str, details: dict) -> None:
        self.connection.execute(
            ...
            (
                collector, event_type, status, datetime.utcnow().isoformat(),
